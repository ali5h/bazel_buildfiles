...
add_package( SANDBOX_ROOT + "openssl-1.0.2e", "openssl")
add_package( SANDBOX_ROOT + "ffmpeg", "ffmpeg")
add_package( SANDBOX_ROOT + "krb5-1.13", "krb5")
# FIX: cross build will fail due to hostcc not supported
add_package( SANDBOX_ROOT + "graphviz-2.38.0", "graphviz")
add_package( SANDBOX_ROOT + "imap-2007e", "imap")
# ------------------------------------------------------------------------------

add_package( REPO_ROOT + "SysUtil/thin-provisioning-tools-v0.4.1", "thin-provisioning-tools")
add_package( REPO_ROOT + "SysUtil/fio-2.2.10", "fio-2.2.10")

add_package( REPO_ROOT + "SysLib/zlib-1.2.8", "zlib")
add_package( REPO_ROOT + "SysLib/pam-0.99.7.1/Linux-PAM", "pam")
add_package( REPO_ROOT + "SysLib/popt-1.14", "popt")
add_package( REPO_ROOT + "SysLib/libid3tag-0.15.1b", "libid3tag")
add_package( REPO_ROOT + "SysLib/json-c-0.9", "json-c")
add_package( REPO_ROOT + "SysLib/sqlite-3.4.1", "sqlite")
add_package( REPO_ROOT + "SysLib/libaio-0.3.109", "libaio")
# add_package( REPO_ROOT + "SysLib/boost_1_53_0", "boost")
...
add_package( REPO_ROOT + "SysLib/expat-2.0.1", "expat")
add_package( REPO_ROOT + "SysLib/xmlrpc", "xmlrpc")
add_package( REPO_ROOT + "SysLib/libxml2-2.9.2", "libxml2")
add_package( REPO_ROOT + "SysLib/eventlog-0.2.7", "eventlog")
# add_package( REPO_ROOT + "SysLib/rlog-1.4", "rlog")
add_package_http("http://pkgs.fedoraproject.org/repo/pkgs/rlog/rlog-1.4.tar.gz/c29f74e0f50d66b20312d049b683ff82/rlog-1.4.tar.gz", "rlog", "a938eeedeb4d56f1343dc5561bc09ae70b24e8f70d07a6f8d4b6eed32e783f79")
add_package( REPO_ROOT + "SysLib/jpeg-6b-src", "jpeg")
add_package( REPO_ROOT + "SysLib/libevent-1.4.12-stable", "libevent")
add_package( REPO_ROOT + "SysLib/libffi-3.2.1", "libffi")
add_package( REPO_ROOT + "SysLib/ncurses-5.5", "ncurses")
add_package( REPO_ROOT + "SysLib/readline-5.2", "readline")
add_package( REPO_ROOT + "SysLib/backtrace", "backtrace")
add_package( REPO_ROOT + "SysLib/gdbm-1.8.3", "gdbm")
add_package( REPO_ROOT + "SysLib/freetype-2.3.7", "freetype")
add_package( REPO_ROOT + "SysLib/libtool-1.5.24", "libtool")
# FIX: libstdc++ incomplete
add_package( REPO_ROOT + "SysLib/libstdcxx-v3", "libstdcxx-v3")
add_package( REPO_ROOT + "SysLib/libupnp-1.6.19", "libupnp")
add_package( REPO_ROOT + "SysLib/libtirpc-0.3.0", "libtirpc")
add_package( REPO_ROOT + "SysLib/libnfsidmap-0.23", "libnfsidmap")
add_package( REPO_ROOT + "SysLib/libgssglue-0.4", "libgssglue")
add_package( REPO_ROOT + "SysLib/librpcsecgss-0.19", "librpcsecgss")
# FIX: libcom_err only currently
add_package( REPO_ROOT + "SysLib/e2fsprogs-1.42.5", "e2fsprogs")
# openldap-2.4.7 have syntax bug, will lead compile error in new toolchain. We use 2.4.33
add_package( REPO_ROOT + "SysLib/openldap-2.4.33", "openldap")
add_package( REPO_ROOT + "SysLib/liboop-1.0", "liboop")
add_package( REPO_ROOT + "SysLib/ruli-0.33", "ruli")
add_package( REPO_ROOT + "SysLib/libusb-1.0.9", "libusb")
add_package( REPO_ROOT + "SysLib/libusb-compat-0.1.4", "libusb-compat")
add_package( REPO_ROOT + "SysLib/postgresql-9.4.4", "postgresql")
add_package( REPO_ROOT + "SysLib/cyrus-sasl-2.1.22", "cyrus-sasl")
add_package( REPO_ROOT + "SysLib/libgpg-error-1.6", "gpg-error")
add_package( REPO_ROOT + "SysLib/libgcrypt-1.4.3", "gcrypt")
add_package( REPO_ROOT + "SysLib/gnutls-2.6.2", "gnutls")
add_package( REPO_ROOT + "SysLib/libtasn1-1.6", "libtasn1")
add_package( REPO_ROOT + "SysLib/bzip2-1.0.6", "bzip2")
# FIX: gold linker did not support unversion symbol. comment out the line in lib/fuse.c is fine
add_package( REPO_ROOT + "SysLib/fuse-2.9.2", "fuse")
add_package( REPO_ROOT + "SysLib/ntfs-3g-1.2412", "ntfs-3g")
add_package( REPO_ROOT + "SysLib/jrtplib-3.3.0", "jrtplib")
add_package( REPO_ROOT + "SysLib/liblinuxfallocate-glibc-2.17", "liblinuxfallocate-glibc")
add_package( REPO_ROOT + "SysLib/libeventfd-glibc-2.17", "libeventfd-glibc")
add_package( REPO_ROOT + "SysLib/libutimensat-glibc-2.17", "libutimensat-glibc")
add_package( REPO_ROOT + "SysLib/tiff-4.0.4", "tiff")
# FIX: original IM have compatible issue with latest zlib, update it
add_package( REPO_ROOT + "SysLib/ImageMagick-6.8.9-9", "ImageMagick")
add_package( REPO_ROOT + "SysLib/attr-2.4.47", "attr")
add_package( REPO_ROOT + "SysLib/acl-2.2.49", "acl")
add_package( REPO_ROOT + "SysLib/gnupg-1.4.19", "gnupg")
add_package( REPO_ROOT + "SysLib/cups-1.6.4", "cups")
#     build_file = "repo/libunistring-0.9.3.BUILD",
#     build_file = "repo/libidn-1.25.BUILD",
add_package( REPO_ROOT + "SysLib/gsoap_2.7.17", "gsoap")
add_package( REPO_ROOT + "SysLib/EtroSDK-src-2.3.3", "EtroSDK-src")
add_package( REPO_ROOT + "SysLib/libcsv-3.0.2", "libcsv")
add_package( REPO_ROOT + "SysLib/librsync-0.9.7", "librsync")
add_package( REPO_ROOT + "SysLib/libmcrypt-2.5.7", "libmcrypt")
add_package( REPO_ROOT + "SysLib/libcommon-0.98.10.1", "libcommon")
add_package( REPO_ROOT + "SysLib/libxslt-1.1.28", "libxslt")
add_package( REPO_ROOT + "SysLib/glib-2.24.2", "glib")
add_package( REPO_ROOT + "SysLib/numactl-2.0.6", "numactl")
#----------------------------------------------------------
